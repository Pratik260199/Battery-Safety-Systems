...
def calculateLCOS(ratedPower, storageDuration, eta_RTE, eta_discharge, eta_charge, capX_energy, capX_power, OM, elecPrice):
    ...
    dutyProfile = pd.read_csv('daysDutyProfile.csv')
    discountRate = 0.1 # 10%
    systemLifetime = 20 # was 20 # years
    discountFactors = 1/((1+discountRate)**np.arange(systemLifetime+1)) # annual discount factors based on specified rate
